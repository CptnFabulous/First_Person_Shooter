...
public class PercentageModifier // Works for both floats and ints
{
    ...
    public bool worksWhileOriginalObjectIsInactive;
    ...
    */
    ...
    public float Calculate()
    {
        ...
        foreach (PercentageModifier fm in influencingPercentages)
        {
            ...
            if (fm.IsActive() && (fm.origin.activeSelf || fm.worksWhileOriginalObjectIsInactive))
            {
                m += fm.percentageValue;
            }
        }

        // Alters defaultValue by the total percentage value
        return defaultValue * (1 + (m / 100));
    }

    public void Add(PercentageModifier pm)
    {
