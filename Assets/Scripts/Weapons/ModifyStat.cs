...
public class StatModifier
{
    public List<StatEffect> appliedEffects = new List<StatEffect>();
    ...
    public void ApplyEffect(string name, float value, float duration)
    {
        if (value != 0) // If statEffect actually has a value, if value is zero it does not modify the stat and is unnecessary
        {
            ...
            appliedEffects.Add(se); // ERROR HERE
        }
    }
    ...
    public float CompileEffects() // Compiles all effect values into a single multiplier
    {
        ...
        foreach (StatEffect se in appliedEffects)
        {
    ...
    public float ModifiedFloat(float f, List<StatEffect> sm) // f equals whatever float you want to alter
    {
    ...
    {
        foreach (StatEffect se in appliedEffects) // (ERROR HERE) Checks each active effect
        {
            //e.duration -= Time.deltaTime; // Counts down duration of effect
            if (se.duration <= 0 || se.value == 0) // If effect duration has expired
            {
                appliedEffects.Remove(se); // Remove from list of active effects
            }
